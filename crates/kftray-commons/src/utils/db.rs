...
mod tests {
    ...
    fn test_init_creates_files_and_db_direct() {
        ...
        if let Ok(cfg_path) = get_db_file_path() {
            ...
            println!("Config path exists? {}", cfg_path.exists());

            if cfg_path == db_path {
                assert!(db_file_exists(), "db_file_exists() should return true");
            } else {
                println!("WARNING: Paths don't match, skipping db_file_exists() assertion");
            }
        }

        if let Ok(manifest_cfg_path) = get_pod_manifest_path() {
            println!("Config manifest path: {}", manifest_cfg_path.display());
            println!("Manifest path exists? {}", manifest_path.exists());

            if manifest_cfg_path == manifest_path {
                assert!(
                    pod_manifest_file_exists(),
                    "pod_manifest_file_exists() should return true"
                );
            } else {
                println!("WARNING: Manifest paths don't match, skipping pod_manifest_file_exists() assertion");
            }
        }
    }
}
