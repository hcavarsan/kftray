...
mod tests {
    ...
    fn test_get_kubeconfig_paths() {
        ...
        let _guard1 = EnvVarGuard::set("KUBECONFIG", custom_kubeconfig_path.to_str().unwrap());
        let kubeconfig_paths = get_kubeconfig_paths().unwrap();
        assert_eq!(kubeconfig_paths, vec![custom_kubeconfig_path.clone()]);
        drop(_guard1);

        let _guard2 = EnvVarGuard::remove("KUBECONFIG");
        let default_home = dirs::home_dir().unwrap();
        let kube_dir = default_home.join(".kube");
